...
public class Unmarshaller {
	private int DEPTH_THRESHOLD = 50;
	...
	Map<String, String> fieldNameClassName = new HashMap<String, String>();
	...
	String adjustFieldNameForXML(String originalText) {
		return originalText.replace("[", "ArrayOf").replaceAll("\\R", " ").replaceAll("\\$", "").replaceAll("<", "")
				.replaceAll(">", "");
	}

	List<Field> findAllClassFields(Class c) {
		if (c == null) {
	...
	String adjustFieldValueForXML(String fieldValue) {
		return fieldValue.replaceAll("<", "").replaceAll(">", "").replaceAll("\\s", " ").replaceAll("\\R", " ")
				.replaceAll("\\&", "").replaceAll("\\x00", "").replaceAll("\\x01", "");
	}
			...
			try {
				if (c.getComponentType().isPrimitive()) {
					for (int i = 0; i < Array.getLength(o); i++) {
						...
						DESTINATION.print("<" + c.getComponentType().getName() + ">"
								+ adjustFieldValueForXML(arrayObject.toString()) + "</" + c.getComponentType().getName()
								+ ">");
					}
	...
	private String printFieldAttributes(Field field, Object fieldObject) {
